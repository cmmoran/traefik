...
func (x Result) Empty() bool {
	return x.country == "" && x.countryCode == "" && x.region == "" && x.city == "" && x.latitude == "" && x.longitude == "" && x.geohash == "" && x.systemNumber == "" && x.systemOrganization == "" && x.network == ""
}

func (x Result) Map() map[string]string {
	if x.Empty() {
	...
	return map[string]string{
		...
		"systemOrganization": x.systemOrganization,
		"network":            x.network,
	}
}
