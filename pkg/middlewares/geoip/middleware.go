...
func (mw *TraefikGeoIP) processRequest(req *http.Request) *http.Request {
	...
	if mw.debug && !result.Empty() {
		logger.Debug().Any("ip", ip).Str("name", mw.name).Any("result", result.Map()).Msg("lookup result")
	} else if mw.debug {
		logger.Trace().Any("ip", ip).Str("name", mw.name).Any("result", result.Map()).Msg("lookup result")
	}
