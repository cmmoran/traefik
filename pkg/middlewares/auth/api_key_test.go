...
func TestAPIKeySecretValuesFile(t *testing.T) {
	t.Parallel()

	secret := hashSecret(t, "mykey")
	tmp, err := os.CreateTemp(t.TempDir(), "apikey")
	if err != nil {
	...
	config := dynamic.APIKey{
		...
		SecretNonBase64Encoded: true,
		SecretValues:           []string{"file://" + tmp.Name()},
	}
...
func TestAPIKeySecretValuesFilePathWithoutScheme(t *testing.T) {
	t.Parallel()
